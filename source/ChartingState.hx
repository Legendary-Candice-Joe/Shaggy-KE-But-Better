...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		m_check.callback = function()
		{
			...
			if (m_check.checked)
			{
				for(section in _song.notes){
					for(i in 0...section.sectionNotes.length){
						if(section.sectionNotes[i][1] > keyAmmo[_song.mania] - 1) section.sectionNotes[i][1] += (6 - keyAmmo[_song.mania]);
					}
				}
				_song.mania = 1;
			} else { //sh********************************************tttttttttttttttttt!!!!!!!!!!!!!!
				for(section in _song.notes){
					for(i in 0...section.sectionNotes.length){
						section.sectionNotes[i][1] = 
						(section.sectionNotes[i][1] > keyAmmo[_song.mania] - 1 ? 4 : 0) + 
						(section.sectionNotes[i][1] % 4);
					}
				}
				_song.mania = 0;
			}
			for(section in _song.notes){
				for(i in 0...section.sectionNotes.length){
					section.sectionNotes[i][1] = 
					(section.sectionNotes[i][1] > keyAmmo[_song.mania] - 1 ? keyAmmo[_song.mania] : 0) + 
					(section.sectionNotes[i][1] % keyAmmo[_song.mania]);
				}
			}
			trace('vos sos puto');
		};
		...
		m_check2.callback = function()
		{
			...
			if (m_check2.checked)
			{
				for(section in _song.notes){
					for(i in 0...section.sectionNotes.length){
						if(section.sectionNotes[i][1] > keyAmmo[_song.mania] - 1) section.sectionNotes[i][1] += (9 - keyAmmo[_song.mania]);
					}
				}
				_song.mania = 2;
			}
	...
	function addSectionUI():Void
	{
		...
		var Randomize:FlxButton = new FlxButton(10, 300, "Random", function()
		{
			var LastRandom = -1;
			
			var OftenVals:Array<Int> = [];
			for (i in 0...keyAmmo[_song.mania])
				OftenVals.push(0);
			
			for (i in 0..._song.notes[curSection].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSection].sectionNotes[i]; // copied lmao
				if (note[1] > -1) {
					var Rand:Int = -1;
					var attemps:Int = 0;
					
					do {
						Rand = Math.round(Math.random() * (keyAmmo[_song.mania] - 1));
					
						OftenVals[Rand] += 3;
						var LowestNumber:Int = 1000; //more than likely, you're not going to use over 1000 notes in each section lmao.
						var LowestID:Int = -1;
						for (X in 0...OftenVals.length){
							if (OftenVals[X] < LowestNumber ) {
								LowestNumber = OftenVals[X];
								LowestID = X;
							}
						}
							
						if (Math.abs(LowestNumber - OftenVals[Std.int(note[1])]) >= (Std.int(Math.random() * 4) + 4))
							Rand = LowestID;

						attemps += 1;
					} while (Math.abs(Rand - LastRandom) <= 2 && attemps <= 10);
						
					if (note[1] > keyAmmo[_song.mania] - 1)
						note[1] = (Rand % keyAmmo[_song.mania]) + keyAmmo[_song.mania];
					else
						note[1] = Rand % keyAmmo[_song.mania];
		...
		tab_group_section.add(Randomize);
