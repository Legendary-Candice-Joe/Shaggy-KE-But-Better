...
class PlayState extends MusicBeatState
{
	...
	function noteHitNotBF(daNote:Note){
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (daNote.isEndHold && FlxG.save.data.downscroll){ //so no weird gaps
						daNote.y += (endNotePositions[mania] / (Conductor.bpm / 64)) * SONG.speed;
					}
					...
					if (daNote.y < -daNote.height && !FlxG.save.data.downscroll || daNote.y >= strumLine.y + 106 && FlxG.save.data.downscroll)
					{
						...
						else if(!HitNotes.contains(Std.string(daNote.strumTime)) && daNote.mustPress)
						{
							health -= 0.075;
							vocals.volume = 0;
							misses++;
		...
		if (!inCutscene && !startingSong) // so the game doesn't crash if you press a key while god eater is starting!
			pooplol();
			//better input.
			//keyShit();
	...
	var Lmaocount:Array<Bool> = [false, false, false, false, false, false, false, false, false];
	
	public function pooplol(){
		if (FlxG.keys.anyJustPressed(Note.KeySet[Note.curKeySet])){
			handleInput();
		}
		playerStrums.forEach(function(spr:FlxSprite)
		{
			if (!FlxG.keys.anyPressed([Note.KeySet[Note.curKeySet][spr.ID]]))
				spr.animation.play('static');
				centerStrums(spr.ID);
		});	
		Lmaocount = [false, false, false, false, false, false, false, false, false];
		notes.forEachAlive(function(daNote:Note){
			if (daNote.isSustainNote && FlxG.keys.anyPressed([daNote.curKeyBind]) && daNote.y >= strumLine.y - 56 && daNote.mustPress && FlxG.save.data.downscroll){
				goodNoteHit(daNote); //probably the laziest sustain code ever lmao
				playerStrums.members[daNote.noteData].animation.play('confirm');
				centerStrums(daNote.noteData);
			} else if (daNote.isSustainNote && FlxG.keys.anyPressed([daNote.curKeyBind]) && daNote.y <= strumLine.y + 56 && daNote.mustPress && !FlxG.save.data.downscroll){
				goodNoteHit(daNote);
				playerStrums.members[daNote.noteData].animation.play('confirm');
				centerStrums(daNote.noteData);
			} else if (Math.abs(daNote.y - strumLine.y) <= 32 && !daNote.mustPress && daNote.isSustainNote) noteHitNotBF(daNote);
			if (HitNotes.contains(Std.string(daNote.strumTime))){
				daNote.alpha = 0;
			}
		});
		if (boyfriend.holdTimer > Conductor.stepCrochet * 0.001 * 4 && boyfriend.animation.curAnim.name.startsWith('sing')
		&& !boyfriend.animation.curAnim.name.endsWith('miss')){ // ^
			boyfriend.dance();                                  // |   bf idle crap.
		}
	}
	
	public var HitNotes:Array<String> = [];
	
	public function handleInput()  //this code is 90% handling strumline animations, 9% crappy code, and 1% somewhat functioning input.
	{
		if (!boyfriend.stunned){
			var UseTheseNotes:Array<Note> = [];
			notes.forEachAlive(function(daNote:Note){
				UseTheseNotes.push(daNote);
			});
			...
			for(daNote in UseTheseNotes){
				if (Math.abs(daNote.strumTime - Conductor.songPosition) <= 95 && FlxG.keys.anyJustPressed([daNote.curKeyBind]) && daNote.mustPress){
		...
		if (HitNotes.length > 10){
			HitNotes = [];
		}
		for (I in 0...Note.KeySet[Note.curKeySet].length){
			if (FlxG.keys.anyPressed([Note.KeySet[Note.curKeySet][I]])){
				if (!playerStrums.members[I].animation.curAnim.name.endsWith('pressed') &&
				!playerStrums.members[I].animation.curAnim.name.endsWith('confirm'))
				{
					playerStrums.members[I].animation.play('pressed');
					centerStrums(I);
					if (!theFunne && !boyfriend.stunned){
						noteMiss(I);
					}
				}
			}
	...
	function noteMiss(direction:Int = 1, ?daNote:Note):Void
	{
		...
		if (daNote != null)
			if (HitNotes.contains(Std.string(daNote.strumTime))) DoIt = false; // to prevent bs at the end of god eater
		
		if (!boyfriend.stunned && DoIt)
		{
			health -= 0.04;
	...
	public function centerStrums(strum:Int)
	{
		playerStrums.members[strum].centerOffsets();
		playerStrums.members[strum].centerOrigin();
	}
	public function centerSHStrums(strum:Int)
	{
		strumLineNotes.members[strum].centerOffsets();
		strumLineNotes.members[strum].centerOrigin();
	}
